...
type Fetcher struct {
	...
	fetchingHook       func([]common.Hash)     // Method to call upon starting a block (ec/61) or header (ec/62) fetch
	completingHook     func([]common.Hash)     // Method to call upon starting a block body fetch (ec/62)
	importedHook       func(*types.Block)      // Method to call upon successful block import (both ec/61 and ec/62)
}
...
func (f *Fetcher) loop() {
	...
	for {
		...
		case notification := <-f.notify:
			...
			if count > hashLimit {
				log.Debug("Peer exceeded ouecanding announces", "peer", notification.origin, "limit", hashLimit)
...
func (f *Fetcher) insert(peer string, block *types.Block) {
	...
	go func() {
		...
		default:
			// Somecing went very wrong, drop the peer
