...
func benchmarkBloomBits(b *testing.B, sectionSize uint64) {
	...
	db, err := ecdb.NewLDBDatabase(benchDataDir, 128, 1024)
	if err != nil {
	...
	for i := 0; i < benchFilterCnt; i++ {
		if i%20 == 0 {
			db.Close()
			db, _ = ecdb.NewLDBDatabase(benchDataDir, 128, 1024)
...
func forEachKey(db ecdb.Database, startPrefix, endPrefix []byte, fn func(key []byte)) {
	it := db.(*ecdb.LDBDatabase).NewIterator()
	it.Seek(startPrefix)
	for it.Valid() {
		key := it.Key()
		cmpLen := len(key)
...
func clearBloomBits(db ecdb.Database) {
