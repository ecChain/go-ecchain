...
import (
	...
	gecmetrics "github.com/ecchain/go-ecchain/metrics"
	"github.com/ecchain/go-ecchain/metrics/influxdb"
...
func Setup(ctx *cli.Context) {
	if gecmetrics.Enabled {
		...
		if enableExport {
			...
			go influxdb.InfluxDBWithTags(gecmetrics.DefaultRegistry, 10*time.Second, endpoint, database, username, password, "swarm.", map[string]string{
				"host": hosttag,
			})
		}
	}
