...
func TestAPIResolve(t *testing.T) {
	...
	tests := []*test{
		...
		},
		{
			...
			dns:       nil,
			addr:      ensAddr,
			expectErr: errors.New(`no DNS to resolve name: "swarm.ec"`),
		},
		{
		...
		},
		{
			...
			addr:      ensAddr,
			immutable: true,
			expectErr: errors.New(`immutable address not a content hash: "swarm.ec"`),
		},
		{
			desc:      "DNS configured, ENS address, name doesn't resolve, returns error",
			dns:       doesntResolve,
			addr:      ensAddr,
			expectErr: errors.New(`DNS name not found: "swarm.ec"`),
		},
	}
	for _, x := range tests {
...
func TestMultiResolver(t *testing.T) {
	...
	ecAddr := "swarm.ec"
	ecHash := "0x2222222222222222222222222222222222222222222222222222222222222222"
	ecResolve := newTestResolver(ecHash)
	...
	tests := []struct {
		...
		},
		{
			...
			r:      NewMultiResolver(MultiResolverOptionWithResolver(ecResolve, "")),
			addr:   ecAddr,
			result: ecHash,
		},
		{
			...
			r: NewMultiResolver(
				MultiResolverOptionWithResolver(ecResolve, ""),
				MultiResolverOptionWithResolver(ecResolve, ""),
			),
			addr:   ecAddr,
			result: ecHash,
		...
		},
		{
			...
			r: NewMultiResolver(
				MultiResolverOptionWithResolver(doesntResolve, ""),
				MultiResolverOptionWithResolver(ecResolve, "ec"),
			),
			addr:   ecAddr,
			result: ecHash,
		},
		{
			...
			r: NewMultiResolver(
				MultiResolverOptionWithResolver(doesntResolve, "ec"),
				MultiResolverOptionWithResolver(doesntResolve, "ec"),
				MultiResolverOptionWithResolver(ecResolve, "ec"),
			),
			addr:   ecAddr,
		...
		},
		{
			...
			addr:   ecAddr,
			result: ecHash,
			err:    errors.New(`DNS name not found: "swarm.ec"`),
		},
		{
