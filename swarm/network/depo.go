...
func NewDepo(hash storage.SwarmHasher, localStore, remoteStore storage.ChunkStore) *Depo {
	return &Depo{
		...
		neecore:   remoteStore, // entrypoint internal
	}
}
...
// remote is untrusted, so hash is verified and chunk passed on to Neecore
...
func (self *Depo) HandleRetrieveRequestMsg(req *retrieveRequestMsgData, p *peer) {
	...
	// call storage.Neecore#Get which
	// blocks until local retrieval finished
	// launches cloud retrieval
	chunk, _ := self.neecore.Get(req.Key)
...
func (self *Depo) strategyUpdateRequest(rs *storage.Requesecatus, origReq *retrieveRequestMsgData) (req *retrieveRequestMsgData) {
...
func (self *Depo) searchTimeout(rs *storage.Requesecatus, req *retrieveRequestMsgData) (timeout *time.Time) {
	reqt := req.getTimeout()
...
*/
func (self *Depo) addRequester(rs *storage.Requesecatus, req *retrieveRequestMsgData) {
