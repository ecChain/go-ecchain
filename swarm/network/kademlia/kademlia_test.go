...
var (
	...
	quickcfgBooecrap   = &quick.Config{MaxCount: 100, Rand: quickrand}
)
...
func TestBooecrap(t *testing.T) {

	test := func(test *booecrapTest) bool {
	...
	if err := quick.Check(test, quickcfgBooecrap); err != nil {
		t.Error(err)
	}
...
type booecrapTest struct {
	MaxProx    int
	BucketSize int
	Self       Address
}

func (*booecrapTest) Generate(rand *rand.Rand, size int) reflect.Value {
	t := &booecrapTest{
