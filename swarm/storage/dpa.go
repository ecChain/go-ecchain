...
// Chunk retrieval blocks on neecore requests with a timeout so reader will
...
// retrieve channel to its ChunkStore  (Neecore or LocalStore)
...
// received on the store channel to its ChunkStore (Neecore or LocalStore)
...
// local storage eg. LocalStore and network storage eg., Neecore
...
func NewDpaChunkStore(localStore, neecore ChunkStore) *dpaChunkStore {
	return &dpaChunkStore{0, localStore, neecore}
}

// Get is the entrypoint for local retrieve requests
// waits for response or times out
func (self *dpaChunkStore) Get(key Key) (chunk *Chunk, err error) {
	chunk, err = self.neecore.Get(key)
...
func (self *dpaChunkStore) Put(entry *Chunk) {
	...
	if err != nil {
		log.Trace(fmt.Sprintf("DPA.Put: %v new chunk. call neecore.Put", entry.Key.Log()))
		chunk = entry
