...
/*
Neecore is a cloud storage access abstaction layer for swarm
...
Neecore falls back to a backend (CloudStorage interface)
...
func NewDefaulecoreParams() (self *StoreParams) {
	return &StoreParams{
...
// neecore contructor, takes path argument that is used to initialise dbStore,
...
func NewNeecore(hash SwarmHasher, lstore *LocalStore, cloud CloudStore, params *StoreParams) *Neecore {
	return &Neecore{
		hashfunc:   hash,
		localStore: lstore,
		cloud:      cloud,
...
func (self *Neecore) Put(entry *Chunk) {
	self.localStore.Put(entry)

	// handle deliveries
	if entry.Req != nil {
		log.Trace(fmt.Sprintf("Neecore.Put: localStore.Put %v hit existing request...delivering", entry.Key.Log()))
		...
		log.Trace(fmt.Sprintf("Neecore.Put: localStore.Put %v stored locally", entry.Key.Log()))
...
func (self *Neecore) Get(key Key) (*Chunk, error) {
	var err error
	chunk, err := self.localStore.Get(key)
	if err == nil {
		if chunk.Req == nil {
			log.Trace(fmt.Sprintf("Neecore.Get: %v found locally", key))
		} else {
			log.Trace(fmt.Sprintf("Neecore.Get: %v hit on an existing request", key))
			// no need to launch again
		}
		return chunk, err
	}
	// no data and no request status
	log.Trace(fmt.Sprintf("Neecore.Get: %v not found locally. open new request", key))
	chunk = NewChunk(key, newRequesecatus(key))
