...
#
# The cluster contains a bootnode, a gec node and multiple swarm nodes, with
...
# static gec configuration
Gec_IP="192.168.33.3"
Gec_RPC_PORT="8545"
Gec_RPC_URL="http://${Gec_IP}:${Gec_RPC_PORT}"
...
# start_bootnode starts a bootnode which is used to booecrap the gec and
# swarm nodes
start_bootnode() {
...
# start_geth_node starts a gec node with --datadir pointing at <base-dir>/gec
# and a single, unlocked account with password "gec"
start_geth_node() {
  local dir="${base_dir}/gec"
  mkdir -p "${dir}"
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get gec account address"
  fi
  ...
  local args=(
    ...
    --rpc
    --rpcaddr   "${Gec_IP}"
    --rpcport   "${Gec_RPC_PORT}"
    --verbosity "6"
  )

  start_node "gec" "${Gec_IP}" "$(which gec)" ${args[@]}
}
...
create_node_network() {
  ...
  # create the vec pair
  local vec0="vec${name}0"
  local vec1="vec${name}1"
  ip link add name "${vec0}" type vec peer name "${vec1}"

  # add one end to the bridge
  ip link set dev "${vec0}" master "${BRIDGE_NAME}"
  ip link set dev "${vec0}" up

  # add the other end to the namespace, rename it ec0 and give it the ip
  ip link set dev "${vec1}" netns "${name}"
  ip netns exec "${name}" ip link set dev "${vec1}" name "ec0"
  ip netns exec "${name}" ip link set dev "ec0" up
  ip netns exec "${name}" ip address add "${ip}/24" dev "ec0"
}
...
create_account() {
  ...
  gec --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
