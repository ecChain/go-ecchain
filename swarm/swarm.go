...
import (
	...
	"github.com/ecchain/go-ecchain/contracts/chequebook"
	"github.com/ecchain/go-ecchain/contracts/ens"
	...
	"github.com/ecchain/go-ecchain/swarm/api"
	httpapi "github.com/ecchain/go-ecchain/swarm/api/http"
	"github.com/ecchain/go-ecchain/swarm/fuse"
	"github.com/ecchain/go-ecchain/swarm/network"
	"github.com/ecchain/go-ecchain/swarm/storage"
)
...
func NewSwarm(ctx *node.ServiceContext, backend chequebook.Backend, config *api.Config) (self *Swarm, err error) {
	...
	self.storage = storage.NewNeecore(hash, self.lstore, self.cloud, config.StoreParams)
...
func newEnsClient(endpoint string, addr common.Address, config *api.Config) (*ens.ENS, error) {
	...
	ensClient := ecclient.NewClient(client)
...
func detectEnsAddr(client *rpc.Client) (common.Address, error) {
	...
	block, err := ecclient.NewClient(client).BlockByNumber(ctx, big.NewInt(0))
	if err != nil {
		return common.Address{}, err
	}
