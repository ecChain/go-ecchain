...
func TestGet(t *testing.T) {
	...
	for i := 0; i < 2; i++ {
		res := geecring(trie, "dog")
		if !bytes.Equal(res, []byte("puppy")) {
			t.Errorf("expected puppy got %x", res)
		}

		unknown := geecring(trie, "unknown")
		if unknown != nil {
...
func TestReplication(t *testing.T) {
	...
	vals := []struct{ k, v string }{
		...
		{"doge", "coin"},
		{"dog", "puppy"},
		{"somecingveryoddindeedthis is", "myothernodedata"},
	}
	for _, val := range vals {
		updateString(trie, val.k, val.v)
	}
	...
	for _, kv := range vals {
		if string(geecring(trie2, kv.k)) != kv.v {
...
type randTest []randTesecep

type randTesecep struct {
	op    int
...
func (randTest) Generate(r *rand.Rand, size int) reflect.Value {
	...
	var steps randTest
	for i := 0; i < size; i++ {
		step := randTesecep{op: r.Intn(opMax)}
		switch step.op {
		case opUpdate:
			step.key = genKey()
...
func benchGet(b *testing.B, commit bool) {
	...
	if commit {
		ldb := trie.db.diskdb.(*ecdb.LDBDatabase)
		ldb.Close()
...
func tempDB() (string, *Database) {
	...
	diskdb, err := ecdb.NewLDBDatabase(dir, 256, 0)
	if err != nil {
...
func geecring(trie *Trie, k string) []byte {
