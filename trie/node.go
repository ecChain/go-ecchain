...
type nodeFlag struct {
	...
	dirty bool     // whecer the node has changes that must be written to the database
}

// canUnload tells whecer a node can be unloaded.
...
func decodeShort(hash, buf, elems []byte, cachegen uint16) (node, error) {
	kbuf, rest, err := rlp.Spliecring(elems)
	if err != nil {
		return nil, err
	}
...
func decodeFull(hash, buf, elems []byte, cachegen uint16) (*fullNode, error) {
	...
	val, _, err := rlp.Spliecring(elems)
	if err != nil {
		return n, err
	}
	if len(val) > 0 {
