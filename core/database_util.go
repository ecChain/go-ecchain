...
var (
	...
	configPrefix   = []byte("ecereum-config-") // config prefix for the db
...
func WriteCanonicalHash(db ecdb.Putter, hash common.Hash, number uint64) error {
...
func WriteHeadHeaderHash(db ecdb.Putter, hash common.Hash) error {
...
func WriteHeadBlockHash(db ecdb.Putter, hash common.Hash) error {
...
func WriteHeadFastBlockHash(db ecdb.Putter, hash common.Hash) error {
...
func WriteTrieSyncProgress(db ecdb.Putter, count uint64) error {
...
func WriteHeader(db ecdb.Putter, header *types.Header) error {
...
func WriteBody(db ecdb.Putter, hash common.Hash, number uint64, body *types.Body) error {
...
func WriteBodyRLP(db ecdb.Putter, hash common.Hash, number uint64, rlp rlp.RawValue) error {
...
func WriteTd(db ecdb.Putter, hash common.Hash, number uint64, td *big.Int) error {
...
func WriteBlock(db ecdb.Putter, block *types.Block) error {
...
func WriteBlockReceipts(db ecdb.Putter, hash common.Hash, number uint64, receipts types.Receipts) error {
...
func WriteTxLookupEntries(db ecdb.Putter, block *types.Block) error {
...
func WriteBloomBits(db ecdb.Putter, bit uint, section uint64, head common.Hash, bits []byte) {
	key := append(append(bloomBitsPrefix, make([]byte, 10)...), head.Bytes()...)
...
func PreimageTable(db ecdb.Database) ecdb.Database {
	return ecdb.NewTable(db, preimagePrefix)
}

// WritePreimages writes the provided set of preimages to the database. `number` is the
// current block number, and is used for debug messages only.
func WritePreimages(db ecdb.Database, number uint64, preimages map[common.Hash][]byte) error {
...
func WriteBlockChainVersion(db ecdb.Putter, vsn int) {
	enc, _ := rlp.EncodeToBytes(uint(vsn))
	db.Put([]byte("BlockchainVersion"), enc)
}

// WriteChainConfig writes the chain config settings to the database.
func WriteChainConfig(db ecdb.Putter, hash common.Hash, cfg *params.ChainConfig) error {
