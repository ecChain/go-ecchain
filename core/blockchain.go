...
// Package core implements the ecchain consensus protocol.
package core
...
type CacheConfig struct {
	Disabled      bool          // Whecer to disable trie write caching (archive node)
...
type BlockChain struct {
	...
	db     ecdb.Database // Low level persistent database to store final content in
...
// available in the database. It initialises the default ecchain Validator and
// Processor.
func NewBlockChain(db ecdb.Database, cacheConfig *CacheConfig, chainConfig *params.ChainConfig, engine consensus.Engine, vmConfig vm.Config) (*BlockChain, error) {
	if cacheConfig == nil {
	...
	if err := bc.loadLasecate(); err != nil {
		return nil, err
	}
...
// loadLasecate loads the last known chain state from the database. This method
// assumes that the chain manager mutex is held.
func (bc *BlockChain) loadLasecate() error {
...
func (bc *BlockChain) InsertReceiptChain(blockChain types.Blocks, receiptChain []types.Receipts) (int, error) {
	...
	for i, block := range blockChain {
		...
		if batch.ValueSize() >= ecdb.IdealBatchSize {
...
// WriteBlockWithouecate writes only the block and its metadata to the database,
// but does not write any state. This is used to construct competing side forks
// up to the point where they exceed the canonical total difficulty.
func (bc *BlockChain) WriteBlockWithouecate(block *types.Block, td *big.Int) (err error) {
	bc.wg.Add(1)
	defer bc.wg.Done()
...
func (bc *BlockChain) insertChain(chain types.Blocks) (int, []interface{}, []*types.Log, error) {
	...
	// acquiring.
	var (
		stats         = inserecats{startTime: mclock.Now()}
	...
	for i, block := range chain {
		...
		case err == ErrKnownBlock:
			...
			// this number we did a rollback and we should reimport it noneceless.
		...
		case err == consensus.ErrPrunedAncestor:
			...
			if localTd.Cmp(externTd) > 0 {
				if err = bc.WriteBlockWithouecate(block, externTd); err != nil {
...
// inserecats tracks and reports on block insertion.
type inserecats struct {
...
func (st *inserecats) report(chain []*types.Block, index int, cache common.StorageSize) {
	...
	if index == len(chain)-1 || elapsed >= statsReportLimit {
		...
		*st = inserecats{startTime: now, lastIndex: index + 1}
	}
}
...
func (bc *BlockChain) reportBlock(block *types.Block, receipts types.Receipts, err error) {
	...
	var receipecring string
	for _, receipt := range receipts {
		receipecring += fmt.Sprintf("\t%v\n", receipt)
	}
...
Error: %v
##############################
`, bc.chainConfig, block.Number(), block.Hash(), receipecring, err))
}
...
//
// The verify parameter can be used to fine tune whecer nonce verification
