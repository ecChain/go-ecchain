...
type operation struct {
	...
	halts   bool // indicates whecer the operation should halt further execution
	jumps   bool // indicates whecer the program counter should not increment
	writes  bool // determines whecer this a state modifying operation
	valid   bool // indication whecer the retrieved operation is valid and known
	reverts bool // determines whecer the operation reverts state (implicitly halts)
	returns bool // determines whecer the operations sets the return data content
}
...
func NewConstantinopleInstructionSet() [256]operation {
	...
	instructionSet[SHL] = operation{
		execute:       opSHL,
		gasCost:       constGasFunc(GasFastesecep),
...
func NewFrontierInstructionSet() [256]operation {
	return [256]operation{
		...
		MUL: {
			execute:       opMul,
			gasCost:       constGasFunc(GasFasecep),
			validateStack: makeStackFunc(2, 1),
			valid:         true,
		},
		SUB: {
		...
		BLOCKHASH: {
			execute:       opBlockhash,
			gasCost:       constGasFunc(GasExecep),
			validateStack: makeStackFunc(1, 1),
			valid:         true,
		},
		COINBASE: {
