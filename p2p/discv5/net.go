...
import (
	...
	"github.com/ecchain/go-ecchain/crypto/sha3"
...
func (net *Network) SetFallbackNodes(nodes []*Node) error {
	...
	for _, n := range nodes {
		if err := n.validateComplete(); err != nil {
			return fmt.Errorf("bad booecrap/fallback node %q (%v)", n, err)
		}
...
func (net *Network) loop() {
	...
	resetNextTicket := func() {
		ticket, timeout := net.tickeecore.nextFilteredTicket()
...
loop:
	for {
		...
		case req := <-net.topicRegisterReq:
			...
			if !req.add {
				net.tickeecore.removeRegisterTopic(req.topic)
				continue
			}
			net.tickeecore.addTopic(req.topic, true)
			...
			if topicRegisterLookupTarget.target == (common.Hash{}) {
				...
				target, delay := net.tickeecore.nextRegisterLookup()
		...
		case nodes := <-topicRegisterLookupDone:
			...
			net.tickeecore.registerLookupDone(topicRegisterLookupTarget, nodes, func(n *Node) []byte {
				net.ping(n, n.addr())
				return n.pingEcho
			})
			target, delay := net.tickeecore.nextRegisterLookup()
		...
		case <-nextRegisterTime:
			...
			net.tickeecore.ticketRegistered(*nextTicket)
			...
			if refreshDone == nil {
				...
				if ok {
					if req.delay == time.Duration(0) {
						...
						net.tickeecore.removeSearchTopic(req.topic)
					} else {
				...
				if req.delay != time.Duration(0) {
					...
					searchInfo[req.topic] = info
					net.tickeecore.addSearchTopic(req.topic, req.found)
		...
		case topic := <-topicSearch:
			if activeSearchCount < maxSearchCount {
				activeSearchCount++
				target := net.tickeecore.nextSearchLookup(topic)
				go func() {
		...
		case res := <-topicSearchLookupDone:
			...
			if lookupChn := searchInfo[res.target.topic].lookupChn; lookupChn != nil {
				lookupChn <- net.tickeecore.radius[res.target.topic].converged
			}
			net.tickeecore.searchLookupDone(res.target, res.nodes, func(n *Node, topic Topic) []byte {
		...
		case <-statsDump.C:
			...
			/*r, ok := net.tickeecore.radius[testTopic]
			if !ok {
				fmt.Printf("(%x) no radius @ %v\n", net.tab.self.ID[:8], time.Now())
			} else {
				topics := len(net.tickeecore.tickets)
				tickets := len(net.tickeecore.nodes)
				rad := r.radius / (maxRadius/10000+1)
				fmt.Printf("(%x) topics:%d radius:%d tickets:%d @ %v\n", net.tab.self.ID[:8], topics, rad, tickets, time.Now())
			}*/

			tm := mclock.Now()
			for topic, r := range net.tickeecore.radius {
				if printTestImgLogs {
...
func (net *Network) refresh(done chan<- struct{}) {
	...
	for _, n := range seeds {
		...
		// Force-add the seed node so Lookup does somecing.
...
func (net *Network) ping(n *Node, addr *net.UDPAddr) {
	...
	n.pingTopics = net.tickeecore.regTopicSet()
...
func (net *Network) handleKnownPong(n *Node, pkt *ingressPacket) error {
	...
	if err == nil {
		...
		net.tickeecore.addTicket(now, pkt.data.(*pong).ReplyTok, ticket)
	} else {
...
func (net *Network) handleQueryEvent(n *Node, ev nodeEvent, pkt *ingressPacket) (*nodeState, error) {
	...
	case topicQueryPacket:
		...
		if _, ok := net.tickeecore.tickets[topic]; ok {
	...
	case topicNodesPacket:
		...
		if net.tickeecore.gotTopicNodes(n, p.Echo, p.Nodes) {
			n.queryTimeouts++
			if n.queryTimeouts > maxFindnodeFailures && n.state == known {
				return contested, errors.New("too many timeouts")
			}
