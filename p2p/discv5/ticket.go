...
func (s *tickeecore) addTopic(topic Topic, register bool) {
...
func (s *tickeecore) addSearchTopic(t Topic, foundChn chan<- *Node) {
	s.addTopic(t, false)
...
func (s *tickeecore) removeSearchTopic(t Topic) {
...
func (s *tickeecore) removeRegisterTopic(topic Topic) {
...
func (s *tickeecore) regTopicSet() []Topic {
...
func (s *tickeecore) nextRegisterLookup() (lookupInfo, time.Duration) {
...
func (s *tickeecore) nextSearchLookup(topic Topic) lookupInfo {
	tr := s.radius[topic]
...
func (s *tickeecore) ticketsInWindow(topic Topic) []ticketRef {
...
func (s *tickeecore) removeExcessTickets(t Topic) {
...
// Less reports whecer the element with
...
func (s *tickeecore) addTicketRef(r ticketRef) {
...
func (s *tickeecore) nextFilteredTicket() (*ticketRef, time.Duration) {
	now := mclock.Now()
	for {
...
func (s *tickeecore) ticketRegistered(ref ticketRef) {
	now := mclock.Now()
...
func (s *tickeecore) nextRegisterableTicket() (*ticketRef, time.Duration) {
	now := mclock.Now()
...
func (s *tickeecore) removeTicketRef(ref ticketRef) {
...
func (s *tickeecore) registerLookupDone(lookup lookupInfo, nodes []*Node, ping func(n *Node) []byte) {
	now := mclock.Now()
	for i, n := range nodes {
...
func (s *tickeecore) searchLookupDone(lookup lookupInfo, nodes []*Node, query func(n *Node, topic Topic) []byte) {
	now := mclock.Now()
	for i, n := range nodes {
		if i == 0 || (binary.BigEndian.Uint64(n.sha[:8])^binary.BigEndian.Uint64(lookup.target[:8])) < s.radius[lookup.topic].minRadius {
			if lookup.radiusLookup {
...
func (s *tickeecore) adjustWithTicket(now mclock.AbsTime, targetHash common.Hash, t *ticket) {
...
func (s *tickeecore) addTicket(localTime mclock.AbsTime, pingHash []byte, ticket *ticket) {
...
func (s *tickeecore) getNodeTicket(node *Node) *ticket {
	if s.nodes[node] == nil {
...
func (s *tickeecore) canQueryTopic(node *Node, topic Topic) bool {
...
func (s *tickeecore) addTopicQuery(hash common.Hash, node *Node, lookup lookupInfo) {
	now := mclock.Now()
	qq := s.queriesSent[node]
	if qq == nil {
...
func (s *tickeecore) cleanupTopicQueries(now mclock.AbsTime) {
...
func (s *tickeecore) gotTopicNodes(from *Node, hash common.Hash, nodes []rpcNode) (timeout bool) {
	now := mclock.Now()
