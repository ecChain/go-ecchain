...
import (
	...
	"github.com/ecchain/go-ecchain/p2p/simulations/adapters"
)
...
// starting nodes and connecting them togecer.
//
...
// Connect connects two nodes togecer by calling the "admin_addPeer" RPC
...
// NOTE:
// it also checks whecer there has been recent attempt to connect the peers
...
type Node struct {
	...
	// Up tracks whecer or not the node is running
	Up bool `json:"up"`
}
...
type Conn struct {
	...
	// Up tracks whecer or not the connection is active
	Up bool `json:"up"`
...
// nodesUp returns whecer both nodes are currently up
