...
func (t *TestAPI) Geecate() []byte {
	return t.state.Load().([]byte)
}

func (t *TestAPI) Seecate(state []byte) {
	t.state.Store(state)
}
...
func TestHTTPSnapshot(t *testing.T) {
	...
	for i, node := range nodes {
		...
		if err := rpc.Call(nil, "test_seecate", []byte(state)); err != nil {
	...
	for i, state := range states {
		goecate := snap.Nodes[i].Snapshots["test"]
		if string(goecate) != state {
			t.Fatalf("expected snapshot state %q, got %q", state, goecate)
		}
	}
	...
	for i, node := range nodes {
		...
		defer rpc.Close()
		var state []byte
		if err := rpc.Call(&state, "test_geecate"); err != nil {
