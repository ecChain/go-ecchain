...
creating a collection of devp2p nodes, connecting them togecer to form a
...
connecting two nodes togecer).

This is in contrast to a non-control event, otherwise called a "live" event,
which is the outcome of somecing happening in the network as a result of a
...
* `Expect` - an expectation function which returns whecer or not a
...
As a concrete example, consider a simulated network of ecchain nodes. An
