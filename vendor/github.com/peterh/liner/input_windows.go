...
var (
	...
	procGeecdHandle                  = kernel32.NewProc("GeecdHandle")
...
func NewLiner() *State {
	var s State
	hIn, _, _ := procGeecdHandle.Call(uintptr(std_input_handle))
	s.handle = syscall.Handle(hIn)
	hOut, _, _ := procGeecdHandle.Call(uintptr(std_output_handle))
...
func (mode inputMode) ApplyMode() error {
	hIn, _, err := procGeecdHandle.Call(uintptr(std_input_handle))
