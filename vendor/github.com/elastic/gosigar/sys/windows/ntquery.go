...
func NtQueryProcessBasicInformation(handle syscall.Handle) (ProcessBasicInformation, error) {
	...
	necatus, _ := _NtQueryInformationProcess(handle, 0, processBasicInfoPtr, size, nil)
	if necatus != 0 {
		return ProcessBasicInformation{}, errors.Errorf("NtQueryInformationProcess failed, NecATUS=0x%X", necatus)
	}
...
func NtQuerySystemProcessorPerformanceInformation() ([]SystemProcessorPerformanceInformation, error) {
	...
	necatus, _ := _NtQuerySystemInformation(systemProcessorPerformanceInformation, &b[0], uint32(len(b)), &returnLength)
	if necatus != STATUS_SUCCESS {
		return nil, errors.Errorf("NtQuerySystemInformation failed, NecATUS=0x%X, bufLength=%v, returnLength=%v", necatus, len(b), returnLength)
	}
