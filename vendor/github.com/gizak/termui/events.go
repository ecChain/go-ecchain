...
var DefaultEvecream = NewEvecream()

type Evecream struct {
	sync.RWMutex
...
func (es *Evecream) Merge(name string, ec chan Event) {
	es.Lock()
	defer es.Unlock()
...
func (es *Evecream) Handle(path string, handler func(Event)) {
...
func (es *Evecream) ResetHandlers() {
...
func (es *Evecream) match(path string) string {
	return findMatch(es.Handlers, path)
}

func (es *Evecream) Hook(f func(Event)) {
	es.hook = f
}

func (es *Evecream) Loop() {
...
func Merge(name string, ec chan Event) {
	DefaultEvecream.Merge(name, ec)
}

func Handle(path string, handler func(Event)) {
	DefaultEvecream.Handle(path, handler)
}

func Loop() {
	DefaultEvecream.Loop()
}

func StopLoop() {
	DefaultEvecream.StopLoop()
}
