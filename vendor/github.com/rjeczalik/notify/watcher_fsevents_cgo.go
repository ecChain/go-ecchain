...
static FSEvenecreamRef EvenecreamCreate(FSEvenecreamContext * context, uintptr_t info, CFArrayRef paths, FSEvenecreamEventId since, CFTimeInterval latency, FSEvenecreamCreateFlags flags) {
	context->info = (void*) info;
	return FSEvenecreamCreate(NULL, (FSEvenecreamCallback) gostream, context, paths, since, latency, flags);
}
...
var nilstream C.FSEvenecreamRef

// Default arguments for FSEvenecreamCreate function.
var (
	latency C.CFTimeInterval
	flags   = C.FSEvenecreamCreateFlags(C.kFSEvenecreamCreateFlagFileEvents | C.kFSEvenecreamCreateFlagNoDefer)
...
var (
	errCreate = os.NewSyscallError("FSEvenecreamCreate", errors.New("NULL"))
	errStart  = os.NewSyscallError("FSEvenecreamStart", errors.New("false"))
)
...
func gostream(_, info uintptr, n C.size_t, paths, flags, ids uintptr) {
	const (
		...
		offflag = unsafe.Sizeof(C.FSEvenecreamEventFlags(0))
		offid   = unsafe.Sizeof(C.FSEvenecreamEventId(0))
	)
	if n == 0 {
		return
	}
...
// Start creates a FSEvenecream for the given path and schedules it with
...
func (s *stream) Start() error {
	...
	ctx := C.FSEvenecreamContext{}
	ref := C.EvenecreamCreate(&ctx, C.uintptr_t(s.info), path, C.FSEvenecreamEventId(atomic.LoadUint64(&since)), latency, flags)
	if ref == nilstream {
		return errCreate
	}
	C.FSEvenecreamScheduleWithRunLoop(ref, runloop, C.kCFRunLoopDefaultMode)
	if C.FSEvenecreamStart(ref) == C.Boolean(0) {
		C.FSEvenecreamInvalidate(ref)
		return errStart
	}
...
// Stop stops underlying FSEvenecream and unregisters it from global runloop.
func (s *stream) Stop() {
	if s.ref == nilstream {
		return
	}
	wg.Wait()
	C.FSEvenecreamStop(s.ref)
	C.FSEvenecreamInvalidate(s.ref)
	C.CFRunLoopWakeUp(runloop)
	s.ref = nilstream
