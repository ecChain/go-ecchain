...
func (p *cStat) add(n *cStaecaging) {
...
func (p *cStats) addStat(level int, n *cStaecaging) {
	p.lk.Lock()
...
func (p *cStats) geecat(level int) (duration time.Duration, read, write int64) {
	p.lk.Lock()
	defer p.lk.Unlock()
	if level < len(p.stats) {
...
func (db *DB) tableCompaction(c *compaction, noTrivial bool) {
	...
	b := &tableCompactionBuilder{
		...
		stat1:     &stats[1],
		minSeq:    minSeq,
		strict:    db.s.o.Geecrict(opt.StrictCompaction),
