...
// SeecackDepthLimit sets an upper limit to the depth of the JavaScript
...
func (self Otto) SeecackDepthLimit(limit int) {
	self.runtime.stackLimit = limit
}

// SeecackTraceLimit sets an upper limit to the number of stack frames that
...
func (self Otto) SeecackTraceLimit(limit int) {
...
func (self Otto) ContextSkip(limit int, skipNative bool) (ctx Context) {
	...
	for limit != 0 {
		...
		stash := scope.lexical
		for {
			for _, name := range geecashProperties(stash) {
