...
func (n *Node) OpenDatabase(name string, cache, handles int) (ecdb.Database, error) {
	if n.config.DataDir == "" {
		return ecdb.NewMemDatabase()
	}
	return ecdb.NewLDBDatabase(n.config.resolvePath(name), cache, handles)
}
