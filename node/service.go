...
func (ctx *ServiceContext) OpenDatabase(name string, cache int, handles int) (ecdb.Database, error) {
	if ctx.config.DataDir == "" {
		return ecdb.NewMemDatabase()
	}
	db, err := ecdb.NewLDBDatabase(ctx.config.resolvePath(name), cache, handles)
	if err != nil {
		return nil, err
	}
	return db, nil
