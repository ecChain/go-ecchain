...
// stateReq represents a batch of state fetch requests groupped togecer into
...
type stateReq struct {
	...
	dropped  bool                       // Flag whecer the peer dropped off early
}
...
func (d *Downloader) runStateSync(s *stateSync) *stateSync {
	...
	for {
		...
		case req := <-timeout:
			// If the peer is already requesting somecing else, ignore the stale timeout.
...
func (s *stateSync) loop() error {
	...
	for s.sched.Pending() > 0 {
		...
		s.assignTasks()
		// Tasks assigned, wait for somecing to happen
		select {
		case <-newPeer:
...
func (s *stateSync) commit(force bool) error {
	if !force && s.bytesUncommitted < ecdb.IdealBatchSize {
		return nil
	}
	start := time.Now()
...
func (s *stateSync) process(req *stateReq) error {
	...
	for hash, task := range req.tasks {
		// If the node did deliver somecing, missing items may be due to a protocol
...
// peer into the state trie, returning whecer anything useful was written or any
// error occurred.
