...
import (
	...
	"github.com/ecchain/go-ecchain/les/flowcontrol"
...
func NewLesServer(ec *ec.ecchain, config *ec.Config) (*LesServer, error) {
	quitSync := make(chan struct{})
	pm, err := NewProtocolManager(ec.BlockChain().Config(), false, ServerProtocolVersions, config.NetworkId, ec.EventMux(), ec.Engine(), newPeerSet(), ec.BlockChain(), ec.TxPool(), ec.ChainDb(), nil, nil, quitSync, new(sync.WaitGroup))
	if err != nil {
		return nil, err
	}
	...
	for i, pv := range AdvertiseProtocolVersions {
		lesTopics[i] = lesTopic(ec.BlockChain().Genesis().Hash(), pv)
	}
	...
	srv := &LesServer{
		...
		quitSync:         quitSync,
		lesTopics:        lesTopics,
		chtIndexer:       light.NewChtIndexer(ec.ChainDb(), false),
		bloomTrieIndexer: light.NewBloomTrieIndexer(ec.ChainDb(), false),
	}
	logger := log.New()
	...
	srv.chtIndexer.Start(ec.BlockChain())
	pm.server = srv
	...
	srv.fcCosecats = newCosecats(ec.ChainDb())
	return srv, nil
}
...
type requestCosecats struct {
	lock  sync.RWMutex
	db    ecdb.Database
	stats map[uint64]*linReg
}

type requestCosecatsRlp []struct {
	MsgCode uint64
	Data    []byte
}

var rcStatsKey = []byte("_requestCosecats")

func newCosecats(db ecdb.Database) *requestCosecats {
	...
	if db != nil {
		...
		var statsRlp requestCosecatsRlp
		if err == nil {
	...
	return &requestCosecats{
		db:    db,
		stats: stats,
	}
}

func (s *requestCosecats) store() {
	s.lock.Lock()
	defer s.lock.Unlock()

	statsRlp := make(requestCosecatsRlp, len(reqList))
...
func (s *requestCosecats) getCurrentList() RequestCostList {
	s.lock.Lock()
	defer s.lock.Unlock()
...
func (s *requestCosecats) update(msgCode, reqCnt, cost uint64) {
	s.lock.Lock()
	defer s.lock.Unlock()
