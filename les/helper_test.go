...
func newTestProtocolManager(lightSync bool, blocks int, generator func(int, *core.BlockGen), peers *peerSet, odr *LesOdr, db ecdb.Database) (*ProtocolManager, error) {
	var (
	...
	if lightSync {
		...
		bloomIndexer := ec.NewBloomIndexer(db, params.BloomBitsBlocks)
	...
	if !lightSync {
		...
		srv.fcCosecats = newCosecats(nil)
	}
	pm.Start(1000)
	return pm, nil
}
...
// fails the test.
func newTestProtocolManagerMust(t *testing.T, lightSync bool, blocks int, generator func(int, *core.BlockGen), peers *peerSet, odr *LesOdr, db ecdb.Database) *ProtocolManager {
