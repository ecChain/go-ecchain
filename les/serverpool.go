...
const (
	...
	// iniecatsWeight is used to initialize previously unknown peers with good
...
type serverPool struct {
	...
	adjusecats          chan poolStatAdjust
...
func newServerPool(db ecdb.Database, quit chan struct{}, wg *sync.WaitGroup) *serverPool {
	pool := &serverPool{
		db:           db,
		quit:         quit,
		wg:           wg,
		...
		adjusecats:  make(chan poolStatAdjust, 100),
...
func (pool *serverPool) disconnect(entry *poolEntry) {
	...
	if entry.state == psRegistered {
		...
		if stopped {
			entry.connececats.add(1, connAdjust)
		} else {
			entry.connececats.add(connAdjust, 1)
		}
	}
...
func (pool *serverPool) adjustBlockDelay(entry *poolEntry, time time.Duration) {
	...
	pool.adjusecats <- poolStatAdjust{pseBlockDelay, entry, time}
}
...
func (pool *serverPool) adjustResponseTime(entry *poolEntry, time time.Duration, timeout bool) {
	...
	if timeout {
		pool.adjusecats <- poolStatAdjust{pseResponseTimeout, entry, time}
	} else {
		pool.adjusecats <- poolStatAdjust{pseResponseTime, entry, time}
	}
}
	...
	for {
		...
		case adj := <-pool.adjusecats:
			...
			case pseResponseTime:
				...
				adj.entry.timeouecats.add(0, 1)
			case pseResponseTimeout:
				adj.entry.timeouecats.add(1, 1)
			}
			pool.lock.Unlock()
...
func (pool *serverPool) findOrNewNode(id discover.NodeID, ip net.IP, port uint16) *poolEntry {
	...
	if entry == nil {
		...
		entry.connececats.add(1, iniecatsWeight)
		entry.delayStats.add(0, iniecatsWeight)
		entry.responseStats.add(0, iniecatsWeight)
		entry.timeouecats.add(0, iniecatsWeight)
	}
...
func (pool *serverPool) loadNodes() {
	...
	for _, e := range list {
		log.Debug("Loaded server stats", "id", e.id, "fails", e.lastConnected.fails,
			"conn", fmt.Sprintf("%v/%v", e.connececats.avg, e.connececats.weight),
			"delay", fmt.Sprintf("%v/%v", time.Duration(e.delayStats.avg), e.delayStats.weight),
			"response", fmt.Sprintf("%v/%v", time.Duration(e.responseStats.avg), e.responseStats.weight),
			"timeout", fmt.Sprintf("%v/%v", e.timeouecats.avg, e.timeouecats.weight))
		pool.entries[e.id] = e
...
type poolEntry struct {
	...
	connececats, delayStats    poolStats
	responseStats, timeouecats poolStats
	state                       int
...
func (e *poolEntry) EncodeRLP(w io.Writer) error {
	return rlp.Encode(w, []interface{}{e.id, e.lastConnected.ip, e.lastConnected.port, e.lastConnected.fails, &e.connececats, &e.delayStats, &e.responseStats, &e.timeouecats})
}
...
func (e *poolEntry) DecodeRLP(s *rlp.Stream) error {
	var entry struct {
		...
		Port                       uint16
		Fails                      uint
		CStat, DStat, RStat, ecat poolStats
	}
	...
	e.connececats = entry.CStat
	e.delayStats = entry.DStat
	e.responseStats = entry.RStat
	e.timeouecats = entry.ecat
...
func (e *knownEntry) Weight() int64 {
	...
	return int64(1000000000 * e.connececats.recentAvg() * math.Exp(-float64(e.lastConnected.fails)*failDropLn-e.responseStats.recentAvg()/float64(responseScoreTC)-e.delayStats.recentAvg()/float64(delayScoreTC)) * math.Pow(1-e.timeouecats.recentAvg(), timeoutPow))
}
