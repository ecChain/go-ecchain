...
func TestSplitTypes(t *testing.T) {
	if _, _, err := Spliecring(unhex("C100")); err != ErrExpectedString {
		t.Errorf("Spliecring returned %q, want %q", err, ErrExpectedString)
	}
	if _, _, err := SplitList(unhex("01")); err != ErrExpectedList {
		t.Errorf("Spliecring returned %q, want %q", err, ErrExpectedList)
	}
