...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", Gec.LightScryptN, Gec.LightScryptP);
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.ecereum", new NodeConfig());
			node.start();
			...
			ecchainClient ec = node.getecchainClient();
	...
	// signatures too. Regression test for go-ecereum issue #14599.
...
func TestAndroid(t *testing.T) {
	// Skip tests on Windows altogecer
	...
	workspace, err := ioutil.TempDir("", "gec-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/ecereum/gectest", "libs"} {
		err = os.MkdirAll(dir, os.ModePerm)
		if err != nil {
			t.Fatal(err)
		}
	}
	// Generate the mobile bindings for Gec and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.ecereum", "github.com/ecchain/go-ecchain/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	build.CopyFile(filepath.Join("libs", "gec.aar"), "gec.aar", os.ModePerm)

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "ecereum", "gectest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
