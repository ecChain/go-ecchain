...
// Package miner implements ecchain block creation and mining.
package miner
...
type Miner struct {
	...
	canStart    int32 // can start indicates whecer we can start the mining operation
	shouldStart int32 // should start indicates whecer we should start after sync
}

func New(ec Backend, config *params.ChainConfig, mux *event.TypeMux, engine consensus.Engine) *Miner {
	miner := &Miner{
		ec:      ec,
		mux:      mux,
		engine:   engine,
		worker:   newWorker(config, engine, common.Address{}, ec, mux),
		canStart: 1,
	}
	miner.Register(NewCpuAgent(ec.BlockChain(), engine))
	go miner.update()
...
func (self *Miner) Start(coinbase common.Address) {
	atomic.StoreInt32(&self.shouldStart, 1)
	self.worker.setecerbase(coinbase)
...
func (self *Miner) Setecerbase(addr common.Address) {
	self.coinbase = addr
	self.worker.setecerbase(addr)
}
