...
type driver interface {
	...
	// Derive sends a derivation request to the USB device and returns the ecchain
...
type wallet struct {
	...
	deriveChain    ecereum.ChainStateReader // Blockchain state reader to discover used account with
...
// whecer they are still present or if they malfunctioned.
...
func (w *wallet) selfDerive() {
	...
	for errc == nil && err == nil {
		...
		for empty := false; !empty; {
			// Retrieve the next derived ecchain account
			...
			// If the next account is empty, stop self-derivation, but add it noneceless
...
// Contains implements accounts.Wallet, returning whecer a particular account is
...
func (w *wallet) SelfDerive(base accounts.DerivationPath, chain ecereum.ChainStateReader) {
	w.stateLock.Lock()
	defer w.stateLock.Unlock()
