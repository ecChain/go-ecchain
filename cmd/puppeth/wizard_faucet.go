...
func (w *wizard) deployFaucet() {
	...
	fmt.Println()
	fmt.Printf("How many ecers to release per request? (default = %d)\n", infos.amount)
	...
	if infos.node.datadir == "" {
		...
		infos.node.datadir = w.readDefaulecring(infos.node.datadir)
	}
	// Figure out which port to listen on
	fmt.Println()
	...
	fmt.Println()
	if infos.node.ecstats == "" {
		fmt.Printf("What should the node be called on the stats page?\n")
		infos.node.ecstats = w.readString() + ":" + w.conf.ecstats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.node.ecstats)
		infos.node.ecstats = w.readDefaulecring(infos.node.ecstats) + ":" + w.conf.ecstats
	}
	...
	infos.noauth = w.readDefaulecring(noauth) != "n"
