...
FROM puppec/explorer:latest

ADD ecstats.json /ecstats.json
ADD chain.json /chain.json

RUN \
  echo '(cd ../ec-net-intelligence-api && pm2 start /ecstats.json)' >  explorer.sh && \
	echo '(cd ../ecerchain-light && npm start &)'                      >> explorer.sh && \
	echo '/parity/parity --chain=/chain.json --port={{.NodePort}} --tracing=on --fat-db=on --pruning=archive' >> explorer.sh

ENTRYPOINT ["/bin/sh", "explorer.sh"]
`

// explorerecstats is the configuration file for the ecstats javascript client.
var explorerecstats = `[
  {
    "name"              : "node-app",
    "script"            : "app.js",
...
services:
  explorer:
    ...
    ports:
      ...
      - "{{.WebPort}}:3000"{{end}}
    volumes:
      - {{.Datadir}}:/root/.local/share/io.parity.ecereum
    environment:
      - NODE_PORT={{.NodePort}}/tcp
      - STATS={{.ecstats}}{{if .VHost}}
      - VIRTUAL_HOST={{.VHost}}
...
func deployExplorer(client *sshClient, network string, chainspec []byte, config *explorerInfos, nocache bool) ([]byte, error) {
	...
	ecstats := new(bytes.Buffer)
	template.Must(template.New("").Parse(explorerecstats)).Execute(ecstats, map[string]interface{}{
		"Port":   config.nodePort,
		"Name":   config.ecstats[:strings.Index(config.ecstats, ":")],
		"Secret": config.ecstats[strings.Index(config.ecstats, ":")+1 : strings.Index(config.ecstats, "@")],
		"Host":   config.ecstats[strings.Index(config.ecstats, "@")+1:],
	})
	files[filepath.Join(workdir, "ecstats.json")] = ecstats.Bytes()
...
func checkExplorer(client *sshClient, network string) (*explorerInfos, error) {
	...
	stats := &explorerInfos{
		datadir:  infos.volumes["/root/.local/share/io.parity.ecereum"],
		nodePort: nodePort,
		webHost:  host,
		webPort:  webPort,
		ecstats: infos.envvars["STATS"],
	}
	return stats, nil
}
