...
func (w *wizard) deployNode(boot bool) {
	...
	if w.conf.Genesis.Config.ecash != nil && !boot {
		fmt.Println()
		if infos.ethashdir == "" {
			...
			infos.ethashdir = w.readDefaulecring(infos.ethashdir)
		}
	}
	...
	if !boot {
		if w.conf.Genesis.Config.ecash != nil {
			// ecash based miners only need an ecerbase to mine against
			fmt.Println()
			if infos.ecerbase == "" {
				fmt.Printf("What address should the miner user?\n")
				for {
					if address := w.readAddress(); address != nil {
						infos.ecerbase = address.Hex()
						break
					}
				}
			} else {
				fmt.Printf("What address should the miner user? (default = %s)\n", infos.ecerbase)
				infos.ecerbase = w.readDefaultAddress(common.HexToAddress(infos.ecerbase)).Hex()
			}
	...
	if out, err := deployNode(client, w.network, w.conf.bootnodes, infos, nocache); err != nil {
		log.Error("Failed to deploy ecchain node container", "err", err)
		if len(out) > 0 {
			fmt.Printf("%s\n", out)
		}
		return
