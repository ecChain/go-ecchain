...
FROM puppec/wallet:latest

ADD genesis.json /genesis.json

RUN \
  echo 'node server.js &'                     > wallet.sh && \
	echo 'gec --cache 512 init /genesis.json' >> wallet.sh && \
	echo $'gec --networkid {{.NetworkID}} --port {{.NodePort}} --bootnodes {{.Bootnodes}} --ecstats \'{{.ecstats}}\' --cache=512 --rpc --rpcaddr=0.0.0.0 --rpccorsdomain "*"' >> wallet.sh

RUN \
	sed -i 's/PuppecNetworkID/{{.NetworkID}}/g' dist/js/ecerwallet-master.js && \
	sed -i 's/PuppecNetwork/{{.Network}}/g'     dist/js/ecerwallet-master.js && \
	sed -i 's/PuppecDenom/{{.Denom}}/g'         dist/js/ecerwallet-master.js && \
	sed -i 's/PuppecHost/{{.Host}}/g'           dist/js/ecerwallet-master.js && \
	sed -i 's/PuppecRPCPort/{{.RPCPort}}/g'     dist/js/ecerwallet-master.js
...
// checkWallet does a health-check against web wallet server to verify whecer
// it's running, and if yes, whecer it's responsive.
