...
func (w *wizard) deployDashboard() {
	...
	for _, service := range []string{"ecstats", "explorer", "wallet", "faucet"} {
		...
		for _, server := range available[service] {
			...
			switch service {
			case "ecstats":
				if infos, err := checkecstats(client, w.network); err == nil {
					port = infos.port
				}
			case "explorer":
	...
	// If we have ecstats running, ask whecer to make the secret public or not
	if w.conf.ecstats != "" {
		fmt.Println()
		fmt.Println("Include ecstats secret on dashboard (y/n)? (default = yes)")
		infos.trusted = w.readDefaulecring("y") == "y"
	}
