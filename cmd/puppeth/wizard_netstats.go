...
func (w *wizard) gatherStats(server string, pubkey []byte, client *sshClient) *serverStat {
	...
	logger.Debug("Checking for ecstats availability")
	if infos, err := checkecstats(client, w.network); err != nil {
		if err != ErrServiceUnknown {
			stat.services["ecstats"] = map[string]string{"offline": err.Error()}
		}
	} else {
		stat.services["ecstats"] = infos.Report()
		ecstats = infos.config
	}
