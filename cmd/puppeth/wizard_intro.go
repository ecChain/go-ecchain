...
// makeWizard creates and returns a new puppec wizard.
...
// setting up a new or managing an existing ecchain private network.
func (w *wizard) run() {
	fmt.Println("+-----------------------------------------------------------+")
	fmt.Println("| Welcome to puppec, your ecchain private network manager |")
	fmt.Println("|                                                           |")
	fmt.Println("| This tool lets you create a new ecchain network down to  |")
	fmt.Println("| the genesis block, bootnodes, miners and ecstats servers |")
	fmt.Println("| without the hassle that it would normally entail.         |")
	fmt.Println("|                                                           |")
	fmt.Println("| Puppec uses SSH to dial in to remote servers, and builds |")
	...
	log.Info("Administering ecchain network", "name", w.network)

	// Load initial configurations and connect to all live servers
	w.conf.path = filepath.Join(os.Getenv("HOME"), ".puppec", w.network)
	...
	for {
		...
		default:
			log.Error("That's not somecing I can do")
		}
	}
}
