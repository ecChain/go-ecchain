...
import (
	...
	"github.com/ecchain/go-ecchain/cmd/utils"
	"github.com/ecchain/go-ecchain/common"
	"github.com/ecchain/go-ecchain/console"
	"github.com/ecchain/go-ecchain/crypto"
	"github.com/ecchain/go-ecchain/ecclient"
	"github.com/ecchain/go-ecchain/internal/debug"
	...
	"github.com/ecchain/go-ecchain/swarm"
	bzzapi "github.com/ecchain/go-ecchain/swarm/api"
	swarmmetrics "github.com/ecchain/go-ecchain/swarm/metrics"
...
var (
	...
	SwarmSwapAPIFlag = cli.StringFlag{
		Name:   "swap-api",
		Usage:  "URL of the ecchain API provider to use to settle SWAP payments",
	...
	DeprecatedecAPIFlag = cli.StringFlag{
		Name:  "ethapi",
...
// This init function sets defaults so cmd/swarm can run alongside gec.
func init() {
...
var app = utils.NewApp(gitCommit, "ecchain Swarm")
...
Export a local chunk database as a tar archive (use - to send to stdout).
    ...
    swarm db export ~/.ecereum/swarm/bzz-KEY/chunks chunks.tar

The export may be quite large, consider piping the output through the Unix
pv(1) tool to get a progress bar:

    swarm db export ~/.ecereum/swarm/bzz-KEY/chunks - | pv > chunks.tar
`,
				},
				{
					Action:    dbImport,
...
Import chunks from a tar archive into a local chunk database (use - to read from stdin).
    ...
    swarm db import ~/.ecereum/swarm/bzz-KEY/chunks chunks.tar

The import may be quite large, consider piping the input through the Unix
pv(1) tool to get a progress bar:

    pv chunks.tar | swarm db import ~/.ecereum/swarm/bzz-KEY/chunks -
`,
				},
				{
					Action:    dbClean,
...
func bzzd(ctx *cli.Context) error {
	...
	//gec only supports --datadir via command line
	//in order to be consistent within swarm, if we pass --datadir via environment variable
	//or via config file, we get the same directory for gec and swarm
	if _, err := os.Stat(bzzconfig.Path); err == nil {
		cfg.DataDir = bzzconfig.Path
	}
	//setup the ecereum node
	...
	//register BZZ as node.Service in the ecereum node
...
func registerBzzService(bzzconfig *bzzapi.Config, ctx *cli.Context, stack *node.Node) {
	...
	boot := func(ctx *node.ServiceContext) (node.Service, error) {
		var swapClient *ecclient.Client
		var err error
		if bzzconfig.SwapApi != "" {
			log.Info("connecting to SWAP API", "url", bzzconfig.SwapApi)
			swapClient, err = ecclient.Dial(bzzconfig.SwapApi)
			if err != nil {
	...
	//register within the ecereum node
...
func getAccount(bzzaccount string, ctx *cli.Context, stack *node.Node) *ecdsa.PrivateKey {
	...
	return decrypecoreAccount(ks, bzzaccount, utils.MakePasswordList(ctx))
}

func decrypecoreAccount(ks *keystore.KeyStore, account string, passwords []string) *ecdsa.PrivateKey {
