...
func TestTopicString(t *testing.T) {
	for i, ec := range topicStringTests {
		s := ec.topic.String()
		if s != ec.str {
			t.Fatalf("failed test %d: have %s, want %s.", i, s, ec.str)
		}
	}
}
...
func TestBytesToTopic(t *testing.T) {
	for i, ec := range bytesToTopicTests {
		top := BytesToTopic(ec.data)
		if top != ec.topic {
			t.Fatalf("failed test %d: have %v, want %v.", i, t, ec.topic)
		}
	}
}

func TestUnmarshalTestsGood(t *testing.T) {
	for i, ec := range unmarshalTestsGood {
		var top TopicType
		err := json.Unmarshal(ec.data, &top)
		if err != nil {
			t.Errorf("failed test %d. input: %v. err: %v", i, ec.data, err)
		} else if top != ec.topic {
			t.Errorf("failed test %d: have %v, want %v.", i, t, ec.topic)
		}
	}
}
...
func TestUnmarshalTestsBad(t *testing.T) {
	...
	for i, ec := range unmarshalTestsBad {
		var top TopicType
		err := json.Unmarshal(ec.data, &top)
		if err == nil {
			t.Fatalf("failed test %d. input: %v.", i, ec.data)
		}
	}
}
...
func TestUnmarshalTestsUgly(t *testing.T) {
	...
	for i, ec := range unmarshalTestsUgly {
		var top TopicType
		err := json.Unmarshal(ec.data, &top)
		if err != nil {
			t.Errorf("failed test %d. input: %v.", i, ec.data)
		} else if top == ec.topic {
			t.Errorf("failed test %d: have %v, want %v.", i, top, ec.topic)
		}
	}
}
