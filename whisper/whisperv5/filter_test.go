...
func TestInstallFilters(t *testing.T) {
	...
	ec := generateTestCases(t, SizeTestFilters)

	var err error
	var j string
	for i := 0; i < SizeTestFilters; i++ {
		j, err = filters.Install(ec[i].f)
		if err != nil {
	...
	for _, testCase := range ec {
		if !testCase.alive {
			filters.Uninstall(testCase.id)
		}
	}

	for i, testCase := range ec {
...
func TestWatchers(t *testing.T) {
	...
	w := New(&Config{})
	filters := NewFilters(w)
	ec := generateTestCases(t, NumFilters)
	for i = 0; i < NumFilters; i++ {
		ec[i].f.Src = nil
		x, err = filters.Install(ec[i].f)
		if err != nil {
	...
	for i = 0; i < NumMessages; i++ {
		...
		e = generateCompatibeEnvelope(t, ec[j].f)
		envelopes[i] = e
		ec[j].msgCnt++
	}
	...
	for i = 0; i < NumFilters; i++ {
		...
		if ec[i].msgCnt != count[i] {
			t.Fatalf("failed with seed %d: count[%d]: get %d, want %d.", seed, i, ec[i].msgCnt, count[i])
		}
	}

	// another round with a cloned filter

	clone := cloneFilter(ec[0].f)
	...
	e = generateCompatibeEnvelope(t, ec[0].f)
	envelopes[0] = e
	ec[0].msgCnt++
	...
	combined := ec[0].msgCnt + ec[last].msgCnt
	...
	for i = 1; i < NumFilters-1; i++ {
		...
		if ec[i].msgCnt != count[i] {
			t.Fatalf("failed with seed %d: i = %d, get %d, want %d.", seed, i, ec[i].msgCnt, count[i])
		}
	}
