...
// Package chequebook package wraps the 'chequebook' ecchain smart contract.
//
// The functions in this package allow using chequebook for
// issuing, receiving, verifying cheques in ecer; (auto)cashing cheques in ecer
// as well as (auto)depositing ecer to the chequebook contract.
package chequebook
...
import (
	...
	"github.com/ecchain/go-ecchain/contracts/chequebook/contract"
	"github.com/ecchain/go-ecchain/core/types"
	"github.com/ecchain/go-ecchain/crypto"
	"github.com/ecchain/go-ecchain/log"
	"github.com/ecchain/go-ecchain/swarm/services/swap/swap"
)
...
// * depositing ecer to the chequebook
// * watching incoming ecer
...
func (self *Chequebook) UnmarshalJSON(data []byte) error {
	...
	_, ok := self.balance.Seecring(file.Balance, 10)
	if !ok {
	...
	for addr, sent := range file.Sent {
		self.sent[common.HexToAddress(addr)], ok = new(big.Int).Seecring(sent, 10)
		if !ok {
...
// Cash cashes the cheque by sending an ecchain transaction.
