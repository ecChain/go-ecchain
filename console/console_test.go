...
func newTester(t *testing.T, confOverride func(*ec.Config)) *tester {
	...
	// Create a networkless protocol stack and start an ecchain service within
	...
	ecConf := &ec.Config{
		...
		ecerbase: common.HexToAddress(testAddress),
		ecash: ethash.Config{
	...
	if err = stack.Register(func(ctx *node.ServiceContext) (node.Service, error) { return ec.New(ctx, ecConf) }); err != nil {
		t.Fatalf("failed to register ecchain protocol: %v", err)
	}
