...
import (
	...
	"github.com/ecchain/go-ecchain/internal/jsre"
	"github.com/ecchain/go-ecchain/internal/web3ext"
...
type Config struct {
	...
	Client   *rpc.Client  // RPC client to execute ecchain requests through
...
func (c *Console) init(preload []string) error {
	...
	c.jsre.Set("jec", struct{}{})

	jecObj, _ := c.jsre.Get("jec")
	jecObj.Object().Set("send", bridge.Send)
	jecObj.Object().Set("sendAsync", bridge.Send)
	...
	if _, err := c.jsre.Run("var web3 = new Web3(jec);"); err != nil {
	...
	//c.jsre.Run(`var GlobalRegistrar = ec.contract(` + registrar.GlobalRegistrarAbi + `);   registrar = GlobalRegistrar.at("` + registrar.GlobalRegistrarAddr + `");`)
	...
	if c.prompter != nil {
		...
		// original web3 callbacks. These will be called by the jec.* methods after
		// they got the password from the user and send the original web3 request to
		// the backend.
		if obj := personal.Object(); obj != nil { // make sure the personal api is enabled over the interface
			if _, err = c.jsre.Run(`jec.openWallet = personal.openWallet;`); err != nil {
				return fmt.Errorf("personal.openWallet: %v", err)
			}
			if _, err = c.jsre.Run(`jec.unlockAccount = personal.unlockAccount;`); err != nil {
				return fmt.Errorf("personal.unlockAccount: %v", err)
			}
			if _, err = c.jsre.Run(`jec.newAccount = personal.newAccount;`); err != nil {
				return fmt.Errorf("personal.newAccount: %v", err)
			}
			if _, err = c.jsre.Run(`jec.sign = personal.sign;`); err != nil {
...
func (c *Console) AutoCompleteInput(line string, pos int) (string, []string, string) {
	...
	// E.g. in case of nested lines ec.getBalance(ec.coinb<tab><tab>
	start := pos - 1
	for ; start > 0; start-- {
...
// Welcome show summary of current Gec instance and some metadata about the
// console's available modules.
func (c *Console) Welcome() {
	// Print some generic Gec metadata
	fmt.Fprintf(c.printer, "Welcome to the Gec JavaScript console!\n\n")
	c.jsre.Run(`
